...
public sealed class CalculateBasalMetabolicRate
{
    ...
    public CalculateBasalMetabolicRate(IMeasurementRepository measurements, ICalculationResultRepository results, IFormulaCatalog catalog, IClock clock)
    {
    ...
    public async Task<Result<CalculationResultDto>> ExecuteAsync(CalculateBmrCommand command, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var bmrFormula = measurement.Gender == ProfileGender.Female
                ? _catalog.FemaleBmr
                : _catalog.MaleBmr;
            var calculated = bmrFormula.Calculate(new BmrInput(measurement.WeightKg, measurement.HeightCm, measurement.AgeYears));
            if (!calculated.IsSuccess)
            {
                return Result.Failure<CalculationResultDto>(calculated.Error!);
            }

            var result = CalculationResult.Create(measurement.Id, CalculationType.BasalMetabolicRate, calculated.Value, _clock.UtcNow);
            if (!result.IsSuccess)
            {
