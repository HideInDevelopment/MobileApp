...
Profile transfer v1 is an offline Application use case exposed by the Android Presentation layer. New exports contain exactly one profile in a password-protected `.anthropometry` file. The file is a versioned AES-256-GCM envelope around the existing UTF-8 CSV payload, with a key derived from the user's passphrase using PBKDF2-HMAC-SHA256. The inner schema contains canonical kilograms, centimetres, UTC timestamps, measurements, and historical calculation results. Display preferences, localized formatting, and external identifiers are never serialized.
...
The Android adapter writes temporary protected exports to `FileSystem.CacheDirectory`, invokes the native Sharesheet through `ShareFileRequest`, and reads imports through `FilePicker`/the Storage Access Framework. It requests no storage permission. Export warns that the file contains personal health data and asks the user to confirm a passphrase without persisting it. Import requires the passphrase before decryption, authenticates the envelope header and ciphertext, and rejects tampering before any database write. Previous plain CSV exports remain a temporary, explicitly labelled legacy import path; new exports never produce CSV files. The portable file deliberately does not use an Android Keystore-bound key, because the recipient device must be able to decrypt it after sharing; anyone who knows the passphrase can create a valid file, so validation and transactional import remain required. Merge/overwrite, multi-profile files, cloud backup, and synchronization remain out of scope.
